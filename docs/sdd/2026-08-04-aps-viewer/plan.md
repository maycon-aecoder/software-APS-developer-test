...
Domain parsing and aggregation retain the safe JavaScript numeric result and approved Area state. Formatting stays in presentation and cannot alter state, unit compatibility, or the calculated value. The aggregate total uses exactly two display decimal places; individual values retain the existing concise display with up to six decimal places. No unit conversion, 15-significant-digit rule, or arbitrary-precision dependency is planned.
...
One feature-local registration factory registers `Aps.ModelQuantities` once per Viewer UI namespace and loads it through the owned Viewer. The extension owns a separate control group, labeled button, resizable `DockingPanel`, and one `Tree`/`TreeDelegate`. The panel is hidden initially, has bounded minimum/maximum dimensions, and keeps native overflow behavior. Button click toggles visibility and native active state; the panel close notification returns the button to inactive. The artificial report root remains structural only and its header is hidden. Doors and Windows are initially collapsed group roots whose multiline native-tree labels include count and total Area state; their explicit triangular indicators align with the first label line and rotate with expansion. Expanding a root renders indented ordered element names and individual Area/`Unavailable` values.

The lifecycle passes a separate deduplicated current-model id list for each supported quantity category into the extension; sanitized quantity results remain id-free. Activating a category or element row invokes only public `viewer.isolate(ids)` and `viewer.fitToView(ids)`. The extension records whether it owns an active isolation and calls `viewer.isolate([])` when the panel closes or resets. It never calls `clearThemingColors`, so category colors remain independently owned. Navigation errors use the extension feedback boundary and expose no ids or raw Viewer error. No export, property-expansion action, additional property read, generic selection framework, or persisted panel state is introduced.

The implementation follows Autodesk's published [custom Tree/TreeDelegate pattern](https://aps.autodesk.com/blog/custom-tree-views) and [extension/DockingPanel pattern](https://aps.autodesk.com/blog/extension-skeleton-toolbar-docking-panel). User-facing Area labels use an explicit allowlist for the Autodesk Forge unit ids observed in this bounded feature (`squareMeters`, `squareFeet`, `squareInches`, and `squareMillimeters`), based on the official [Revit `UnitTypeId` Area members](https://help.autodesk.com/cloudhelp/2026/ENU/Revit-API-MainReference/files/html/bc1b6454-f10a-66dc-9268-1dccbc403f78.htm) and [ForgeTypeId overview](https://help.autodesk.com/cloudhelp/2024/ESP/Revit-API/files/Revit_API_Developers_Guide/Introduction/Application_and_Document/Revit_API_Revit_API_Developers_Guide_Introduction_Application_and_Document_ForgeTypeId_html.html). Unknown Autodesk unit ids are unavailable rather than guessed or displayed as internal codes.

The lifecycle coordinator owns only the loaded extension reference. It resets the extension before old model unload, forwards deduplicated category ids through the internal navigation channel and sanitized current-generation quantity results through the presentation channel, and releases its reference before `viewer.finish()`. URN-only replacement reuses the extension; credential replacement relies on Viewer `finish()` to invoke extension `unload()`, then loads exactly one new instance after reinitialization. Setup/update failures log only stable codes, keep geometry usable, and surface the existing model retry action. The page-level React quantity report is removed; React retains only Viewer status and retry feedback.

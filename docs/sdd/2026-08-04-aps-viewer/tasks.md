...
- [x] `T-008` Implement instance/category resolution and native toolbar/theming test-first
  ...
  - Depends on: `T-007`.
  - Red evidence: Controlled fixtures require exact supported direct-root category nodes and unique recursive leaf `dbId` results; similar root labels and matching deeper nodes do not qualify. Tests cover nested intermediate levels, duplicate callbacks, truthful sibling enumeration, malformed roots/cycles, no element `Category` property read, and model-local reset. Toolbar tests require one native group/control per Furniture/Walls/Doors, readiness/failure isolation, keyboard labels/focus retention, pointer toggle-off returning to unfocused inactive state, zero-match feedback, distinct semantic active state, and no stale ids. Theming tests use active-model public calls, turn one category off by clearing/rebuilding feature-owned ids while preserving others, cover overlapping sets, and clear all owned theming on replacement/teardown.
  ...
  - Approval checkpoint: Review root/category/leaf fixtures, safe tree failure, public theming calls, pointer-versus-keyboard state, overlap behavior, and cleanup before Green.
  - Minimum Green: Implement the narrow public root-category/recursive-leaf adapter contract, exact resolver, and feature toolbar controller with active-category map and feature-owned `dbId` set.
  - Refactor boundary: Exact tree traversal stays inside the adapter; do not read an element property named `Category`, promote deeper labels, infer translations, use fuzzy matching, touch private fragments, or clear unrelated theming.
...
- [x] `T-009` Implement quantity domain and progressive property orchestration test-first
  ...
  - Depends on: `T-008`.
  - Red evidence: Pure tables require unique Door/Window leaf counts including zero; exact Area name; approved whole-string numeric grammar; rejection of booleans, negatives, non-finite, exponent, grouping, partial text, and inference; trim-only units; compatibility/no conversion; and all report states. Multiple Area fixtures use only `dbId`, `displayName`, `displayValue`, `displayCategory`, `attributeName`, `type`, `units`, `hidden`, and `precision`. They contribute once only when every record is valid and identical in numeric value/unit/non-empty stable `attributeName`/`type`; same display category alone is insufficient; missing metadata is ambiguous. Async tests require root-category leaves before bounded Area phases, count before Area, per-category isolation, Area skip after unsafe count, count retention/no subtotal on operational Area failure, and generation guards for every batch/result/error.
  ...
  - Refactor boundary: Separate public property adaptation, pure domain, orchestration, and formatting; no arbitrary record selection, alternate category inference, unit conversion/inference, 15-significant-digit rule, precision library, or partial subtotal on operational failure.
...
- [x] `T-010A` Replace the page report with a native quantity extension and element detail test-first
  ...
  - Red evidence: Extend adapter/domain/controller tests for trimmed public names, deterministic ordinal fallback, per-element contribution outcomes, no `dbId`/raw properties, and aggregate/detail consistency. Add controlled Viewer tests for one separately registered extension/group/button, resizable `DockingPanel` plus `Tree`/`TreeDelegate`, hidden artificial root, initially collapsed aligned Door/Window roots, multiline summary labels, indented ordered element rows, two-decimal aggregate display, category/item isolate-and-fit, panel-owned isolation cleanup without theming cleanup, button/close state synchronization, keyboard operation, reset, stale suppression, update/setup failure, and idempotent unload. Replace React report assertions with absence below Viewer and retained retry/status behavior.
  ...
  - Minimum Green: Retain the public result `name`, produce safe ordered element detail from the existing Area contribution resolver, register/load one `Aps.ModelQuantities` extension, render the two native-tree roots in one bounded resizable docking panel, pass current category ids through a separate internal channel, isolate/fit activated rows through public Viewer APIs, clear only panel-owned isolation on close/reset, wire lifecycle reset/update/unload, and remove only the page-level report component/wiring.
  - Refactor boundary: Share contribution evaluation between aggregate and detail; keep analysis outside the extension. Do not refactor color controls, clear theming from the quantity extension, introduce a generic extension framework, React portal, dependency, extra property browser, persistence, or Viewer-private API.
...
- Model realism: Controlled fixtures deliberately contain exact and similar root labels, intermediate nodes, nested leaves, repeated callbacks, and malformed trees. Real sanitized Revit evidence is mandatory because doubles cannot prove derivative structure or visible theming.

...
- Category analysis inspects only direct children of the active model's public instance-tree root. It accepts the eight exact supported aliases, optionally removing only a trailing structural count ` (<digits>)`, and publishes the unique recursive leaf `dbId` values beneath each matched category. It never reads an element property named `Category`, promotes a matching deeper node, or uses private fragment data. Repeated callbacks are deduplicated; malformed roots or cycles fail safely instead of reporting zero.
- One feature-owned native toolbar group exposes independently available Furniture, Walls, and Doors controls with scoped Google Fonts Material Symbols (`chair`, `foundation`, and `door_front`), full tooltips, and accessible labels/state. It applies active-model theming with distinct colors, rebuilds overlapping feature-owned colors on deactivation, clears state on model replacement, releases native ownership on teardown, and surfaces safe retry feedback when analysis or Viewer color operations fail.
- Category analysis results and toolbar mutations pass through the lifecycle coordinator's active operation guard; obsolete model analysis cannot publish state or stale `dbId` values. The quantity domain consumes only resolved Door/Window leaf `dbId` sets, publishes deduplicated counts before bounded visible-property reads, evaluates exact Area records and units deterministically, and suppresses obsolete batches/results/errors. A separate `Aps.ModelQuantities` extension owns the native command, resizable docking panel, and safe Door/Window tree. Sanitized aggregate/detail results remain id-free; a separate current-model id channel supports public isolate-and-fit actions. Closing the panel clears only its owned isolation and leaves category theming unchanged, while incomplete or failed analysis preserves the usable Viewer.

...
- Category analysis discovers exact Revit category containers along pre-geometry organizational branches, allowing only the structural count suffix ` (<digits>)`, then selects the first renderable descendant on each category branch and confirms its exact `Category` property before publishing a model-local instance `dbId`. Exact category names take precedence over fragment evidence because Revit category containers may expose aggregated child fragments. The adapter stops at the first renderable descendant, so non-leaf instances remain eligible while nested geometry is not reused as another instance. Missing or contradictory identity evidence fails the affected category instead of reporting zero.
